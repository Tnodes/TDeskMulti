...
layout = [
    [sg.Button(strings['new_account']), sg.Button(strings['update_tdesk'])],
    [sg.Listbox(values=list(accounts.keys()), size=(40, 10), bind_return_key=True, key='selected_account'),
     sg.Column([[sg.Button(strings['start'])], [sg.Button(strings['edit_name'])], [sg.Button(strings['delete_account'])]])]
]
    ...
    if event == strings['new_account']:
        ...
        if name:
            if name not in accounts:
                ...
                window['selected_account'].update(list(accounts.keys()))
